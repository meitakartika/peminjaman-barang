...
const Barang = () => {
  ...
  const deleteBarang = async (barangId) => {
    try {
      ...
      console.error("Error deleting barang:", error.response?.data?.msg || error.message);
    }
  };
  ...
  const totalPages = Math.ceil(filteredBarang.length / entriesPerPage);
  const paginatedBarang = filteredBarang.slice((currentPage - 1) * entriesPerPage, currentPage * entriesPerPage);
            ...
            <div className="page-title" style={{ marginBottom: '20px' }}>
              <div className="row">
                <div className="col-12 col-md-6 order-md-1 order-last">
                  <h4 style={{ color: '#243561' }}>Data Barang</h4>
                </div>
              </div>
            </div>
            <section className="section">
              <div className="card" style={styles.card}>
                <div className="card-header" style={styles.cardHeader}>
                  <Link to="/barang/add" className='btn btn-sm btn-primary' style={styles.addButton}>
                ...
                <div className="card-body">
                  <div className="table-responsive">
                    ...
                    <table className="table" id="table1" style={{ borderColor: '#dee2e6' }}>
                      ...
                      <tbody>
                        {paginatedBarang.length > 0 ? (
                          paginatedBarang.map((barang, index) => (
                            <tr key={barang.id}>
                              <td style={{ color: 'black' }}>{(currentPage - 1) * entriesPerPage + index + 1}</td>
                              <td style={{ color: 'black' }}>{barang.kode}</td>
                              <td style={{ color: 'black' }}>{barang.name}</td>
                              <td style={{ color: 'black' }}>{barang.stok}</td>
                              <td>
                                <Link to={`/barang/detail/${barang.id}`} className="btn btn-sm btn-secondary">
                                  <i className="fa fa-bars"></i> Detail
                                </Link>
                                <Link to={`/barang/edit/${barang.id}`} className="btn btn-sm btn-primary ms-2">
                                  <i className="fa fa-edit"></i> Edit
                                </Link>
                                <button onClick={() => deleteBarang(barang.id)} className="btn btn-sm btn-danger ms-2">
                    ...
                    <div className="d-flex justify-content-between" style={{ color: 'black' }}>
                      <div style={{ color: 'black' }}>Showing {paginatedBarang.length} of {filteredBarang.length} entries</div>
                      <div>
                        <button
